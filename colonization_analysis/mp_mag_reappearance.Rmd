...
Setting up a zero-inflated beta regression for reappeared relative abundance. 
```{r}
# GENERATE PREDICTED RELATIVE ABUNDANCES AFTER AN ABSENCE
for (i in 1:2) {
  zoib.model <- zoib.list[[i]]
  coef.summary <- summary(zoib.model$coeff)
  coefs <- coef.summary$statistics[ , 1]
  
  # select the dataframe
  df <- df.list[[i]]
  # predicted zero-inflated reappeared relative abundances
  df$pred <- sim.zoib(zoib.model = zoib.model, beta.mean.coefs = coefs[1:3], infl.coefs = coefs[4], beta.disp.coefs = coefs[5:6], infl = 0, seed = 0)
  # update the list of dataframes
  df.list[[i]] <- df
}

# Coefficients are presented in the order of b, b0 (if zero.inflation=TRUE), b1 (if one.inflation=TRUE), and d
z <- zoib(model = y ~ median.abundance + log(sample.read.depth) | median.abundance | 1, 
          data = new.reapp.relabs.f4, joint = FALSE, n.response = 1,
          zero.inflation = TRUE, one.inflation = FALSE, 
          EUID = 1:nrow(new.reapp.relabs.f4),
          n.iter = 500)
...
# 
# ggplot(reapp.relabs.m3) +
#   geom_point(aes(x = log(sample.read.depth), y = y)) +
#   labs(y = "Reappeared Relative Abundances")
# 
# ggplot(reapp.relabs.f4) +
#   geom_boxplot(aes(y)) +
#   labs(title = "F4", x = "Reappeared Relative Abundances")
# 
# ggplot(reapp.relabs.m3) +
#   geom_boxplot(aes(y)) +
#   labs(title = "M3", x = "Reappeared Relative Abundances")
