...
foreach(set = 1:n.set) %dopar% simDM(set, n.subj, dd.DR, "./SimSets_DR_v01")
...
reapp.means <- predict(reapp.beta, newdata = otu.rarity.newdata, allow.new.levels = TRUE, type = "response")
reapp.disps <- predict(reapp.beta, newdata = otu.rarity.newdata, allow.new.levels = TRUE, type = "disp")
...
simLong <- function(set, n.otu, n.subj, n.time, 
  ...
  for (tt in 2:n.time) {
    ...
    perturb.lval <- sim.log.foldchange(otu.quintiles, n.subj, base.sd = 0.75, scaling.factors = c(.5, .75, 1, 1.25, 5))
