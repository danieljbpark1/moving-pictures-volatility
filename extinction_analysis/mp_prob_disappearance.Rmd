...
sim.disapp.tab <- rbind(prob.disapp.tab(otutab = sim.otutab.f4, subj.id = "F4"),
                        prob.disapp.tab(otutab = sim.otutab.m3, subj.id = "M3"))

head(sim.disapp.tab)

ggplot(sim.disapp.tab, aes(x=avg.asin, y=prob.disappear, color=subj.id)) +
  geom_point() +
  labs(title = "OTU Probabilities of Disappearance",
       subtitle = paste("Simulated Moving Pictures dataset set", sprintf("%04d", set), sep = ""),
       x = "average within-subject asin Hellinger abundance", 
...
disapp.plot <- ggplot(disapp.tab.ALL, aes(x = avg.asin, y = prob.disappear)) +
